...
// Function to save WPM and Score to the database
function saveGameData(wpm_account, score_account) {
    fetch('save_wpm.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
        body: JSON.stringify({ wpm_account, score_account }), // ส่งข้อมูลในรูปแบบ JSON
    })
        .then(response => response.json())
        .then(data => {
            if (data.status === 'success') {
                console.log('Data saved successfully');
                updateMetrics(); // Fetch updated data and display it
            } else {
...
// Function to fetch updated metrics from the session
function updateMetrics() {
    fetch('fetch_metrics.php') // Create a PHP file to return session data
        .then(response => response.json())
        .then(data => {
            if (data.status === 'success') {
                const wpmElement = document.querySelector('.metrics span:nth-child(1)');
                const scoreElement = document.querySelector('.metrics span:nth-child(2)');
                wpmElement.textContent = data.wpm_account || '0';
                scoreElement.textContent = data.score_account || '0';
            } else {
...
function endGame() {
    ...
    //const score = updateScore(); // Calculate score

    // Save data to the database
    saveGameData(wpm_account, score_account);
